! This is the version of the solver before changing the interpolation procedure
! from triangle centers to triangle vertices. Lines changed were:
! 192-210  during the solution steps;
! 410-429  for printing intermediate solutions;
! 516-535  for final output.
...
!  Last updated (mm-dd-yyyy): 07-17-2012 by F. Simoes                         !
!                                                                             !
!-----------------------------------------------------------------------------!
...
! Set up initial values for RK stepping.
  ...
  DO
    ...
    DO rkstep = 1,rkorder
      ...
      ! Enforce boundary conditions here to ensure that the vertex variables
      ! have the proper values.
      CALL enforce_bcs(hvtx,uvtx,vvtx,zvtx,n_pts)
      CALL zeta_bcs
      IF (ANY(drycell)) THEN
        CALL wetdryfix(1,hvtx,h,delh)  ! Fix nodes at wet-dry boundaries.
        IF (btype == 0) CALL wetdryvfix
      END IF
      WHERE (hvtx < h_dry)  ! Enforce zero velocity at dry nodes.
        uvtx = zero
        vvtx = zero
      END WHERE
      CALL positivity_vtx
      ...
      ! Explicit implementation of the bed friction terms.  Some of the info
      ! computed here is used in the eddy viscosity computations.
      !CALL fric_terms
    ...
    ELSE  ! March in time.
      ...
      WRITE (*,'(I8,11ES13.4)') iter,stime,sumbot,tmass, &
...
! necessary to output the vertex values (for iRIC, for example).
  ...
  CALL ctr2vtx(nctr2vtx,v,vvtx)
  CALL cell_grads
  CALL hgradfix(1,opt_solver)  ! Sets zero grad of stage at wall triangles.
  CALL enforce_bcs(hvtx,uvtx,vvtx,zvtx,n_pts)
  CALL zeta_bcs
  IF (ANY(drycell)) THEN
    CALL wetdryfix(1,hvtx,h,delh)  ! Fix nodes at wet-dry boundaries.
