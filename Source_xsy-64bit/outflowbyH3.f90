...
!  Enforces a clamped boundary condition.                                     !
!                                                                             !
!  Francisco Simoes, September 2008                                           !
...
! Local variables.
  ...
  DO icycle = 1,n_outflowbdr
    ...
    DO l = 1,bcedgesoutflow(0,icycle)
      ...
      h(i) = MAX(hbc(icycle) - z(i),zero)
      zmiddle = half*(zvtx(ework%p(1)) + zvtx(ework%p(2)))
      hleft = hbc(icycle) - zmiddle
      ...
      ! Debugging discharges.
      !cqout = cqout + (uright*ework%normal(1) + vright*ework%normal(2))* &
      !        hright*ework%length
      !cqout2 = cqout2 + (uleft*ework%normal(1) + vleft*ework%normal(2))* &
      !         hleft*ework%length
      cqout3 = cqout3 + f(1)*ework%length
